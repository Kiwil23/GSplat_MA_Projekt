...
#  pre_filter_img ist entweder False (nicht gesetzt), True (nur Flag übergeben), oder float
if args.pre_filter_img is False:
    pre_filter_img = False
elif args.pre_filter_img is True:
    print("Bitte gib einen Zahlenwert für --pre_filter_img an, z.B. 80.0")
    exit(1)
else:
    pre_filter_img = args.pre_filter_img / 100.0  

# train_img_percentage ist entweder False (nicht gesetzt), True (nur Flag übergeben), oder float
if args.train_img_percentage is False:
    train_img_percentage = False
elif args.train_img_percentage is True:
    print("Bitte gib einen Zahlenwert für --train_img_percentage an, z.B. 80.0")
    exit(1)
else:
    train_img_percentage = args.train_img_percentage / 100.0  

#  post_filter_img ist entweder False (nicht gesetzt), True (nur Flag übergeben), oder float
if args.post_filter_img is False:
    post_filter_img = False
elif args.post_filter_img is True:
    print("Bitte gib einen Zahlenwert für --post_filter_img an, z.B. 80.0")
    exit(1)
else:
...
def extract_useful_images(in_dir, out_dir):
    temp_input_dir = ""
    # Does a pre cleanup of blury images and reduces count of images but can fail on small datasets
    if isinstance(pre_filter_img, float):
        echo(f"\033[92mRemoving the worst 5'%' images.\033[0m")
        # Decides if input is Video or Images
    ...
    # Extracs sharp and overlaping images from mp4 or imagefolder
    ...
    # Does a extra cleanup of blury images and reduces count of images but can fail on small datasets
...
def prepare_colmap_data_for_splatfacto(in_dir, out_dir, colmap_dir):
    ...
    if isinstance(train_img_percentage, float):
        ...
        print(f"✅ Registrierte Bilder: {len(frames)}")
...
#---------------------------------------------------------------------------------------------------------------------------
print(f"\033[92mStarting pipeline: {pipeline_type} with pre_filter_img set to: {pre_filter_img*100} and post_filter_img set to: {post_filter_img*100} an train_img_precentag set to: {train_img_percentage*100}\033[0m")
